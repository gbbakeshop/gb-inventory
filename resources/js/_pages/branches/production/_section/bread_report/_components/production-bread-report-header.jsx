...
import { filteredBreadData } from "../../../_redux/production-slice";

export default function ProductionBreadReportHeader() {
    const [query, setQuery] = useState("");
    const { breadData } = useSelector((state) => state.production);
    const dispatch = useDispatch();

    const filtered =
        query === ""
            ? breadData
            : breadData.filter((recipe) =>
                  recipe.bread.name
                      .toLowerCase()
                      .replace(/\s+/g, "")
    ...
    useEffect(() => {
        dispatch(filteredBreadData(filtered));
    }, [query]);
    return (
