...
export default function BranchSideNavbar({subNav}) {
  const { url } = usePage();
  ...
  return (
      <aside className="flex top-0 sticky ">
          <div className="flex flex-col items-center w-16 h-screen py-8 space-y-8 bg-white  ">
              ...
              <Link href={route("branch.raw_materials")} className={active("raw_materials")}>
                  <TruckIcon className="w-6 h-6" />
              </Link>

              <Link
                  href={route("branch.production")}
                  className={active("production")}
              >
                  <CakeIcon className="w-6 h-6" />
              </Link>

              <Link  href={route("branch.selecta")}className={active("selecta")}>
