...
import { setFilteredRawMaterials } from "../_redux/raw-materials-slice";

export default function BranchRawMaterialsTableHeader() {
  const [query, setQuery] = useState("");
  const { rawMaterials } = useSelector((state) => state.raw_materials);
  const dispatch = useDispatch();
  const filtered =
      query === ""
          ? rawMaterials
          : rawMaterials.filter((res) =>
  ...
  useEffect(() => {
      dispatch(setFilteredRawMaterials(filtered));
  }, [query]);
  return (
