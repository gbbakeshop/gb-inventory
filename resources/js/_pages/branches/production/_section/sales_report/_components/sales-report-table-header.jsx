...
import { filteredSalesData } from "../../../_redux/production-slice";

export default function SalesReportTableHeader() {
  const [query, setQuery] = useState("");
  const { salesData } = useSelector((state) => state.production);
  const dispatch = useDispatch();

  const filtered =
      query === ""
          ? salesData
          : salesData.filter((recipe) =>
                recipe.bread.name
                    .toLowerCase()
                    .replace(/\s+/g, "")
  ...
  useEffect(() => {
      dispatch(filteredSalesData(filtered));
  }, [query]);
  return (
