...
import { filteredBakersData } from "../../../_redux/production-slice";

export default function ProductionBakersReportHeader() {
    const [query, setQuery] = useState("");
    const { bakersData } = useSelector((state) => state.production);
    const dispatch = useDispatch();

    const filtered =
        query === ""
            ? bakersData
            : bakersData.filter((recipe) =>
                  recipe.bread.name
                      .toLowerCase()
                      .replace(/\s+/g, "")
    ...
    useEffect(() => {
        dispatch(filteredBakersData(filtered));
    }, [query]);
    return (
