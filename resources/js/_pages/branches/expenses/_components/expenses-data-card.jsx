...
import TableCharge from "./table-charge";
...
export default function ExpensesDataCard({ data, account, branch_id }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const branchAccountRes = await get_branch_account({
                    branch_id: branch_id,
                });
                setUsers(branchAccountRes.status);

                const allBreadRes = await get_all_bread();
                setBreads(allBreadRes.status);

                const allSelectaRes = await get_all_selecta();
                setSelecta(allSelectaRes.status);

                const rawMaterialsRes = await get_all_raw_materials();
                setRawMaterials(rawMaterialsRes.status);
